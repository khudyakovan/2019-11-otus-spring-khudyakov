...
CREATE TABLE `tbl_comments` (
  ...
  CONSTRAINT `tbl_comments_FK` FOREIGN KEY (`commentator_uid`) REFERENCES `tbl_commentators` (`uid`) ON DELETE CASCADE ON UPDATE CASCADE
...
--
-- Table structure for table `tbl_book_comments`
--

CREATE TABLE `tbl_book_comment` (
  `book_uid` bigint(20) NOT NULL,
  `comment_uid` bigint(20) NOT NULL,
  KEY `tbl_book_comments_FK` (`book_uid`),
  KEY `tbl_book_comments_FK_1` (`comment_uid`),
  CONSTRAINT `tbl_book_comments_FK` FOREIGN KEY (`book_uid`) REFERENCES `tbl_books` (`uid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `tbl_book_comments_FK_1` FOREIGN KEY (`comment_uid`) REFERENCES `tbl_comments` (`uid`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Регистр комментариев к книгам';
