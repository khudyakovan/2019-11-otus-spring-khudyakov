...
class GenreServiceImplTest {
    ...
    private static final String NEW_GENRE_UID = "100500";
    private static final String NEW_GENRE_NAME = "Test Genre Name";
    private static final String TEST_GENRE_UID = "2";
    ...
    void setUp() {
        ...
        given(genreRepository.findById(anyString())).willReturn(Optional.ofNullable(genre));
        given(genreRepository.findGenresByBookUid(anyString())).willReturn(genres);
        given(genreRepository.count()).willReturn(Long.parseLong(NEW_GENRE_UID));
    }

    @DisplayName("...должен найти запись по ее Uid")
    @Test
    void shouldFindByUid() {
        assertThat(genreService.findByUid(TEST_GENRE_UID)).isNotNull();
    }
